...
#   - CLICK: Audio click at each second boundary, synchronized to the flash —
#            film screen+speaker together and count the frame gap between the
#            visual flash and the audible click to measure audio output latency
#            (e.g. Bluetooth A2DP delay).
#
...
# Click track: short sine burst at each second boundary (t=0.000, 1.000, 2.000, ...),
# aligned with the start of the visual flash so the two can be compared frame-by-frame.
CLICK_FREQ=1000
CLICK_MS=20
CLICK_AMP=0.8
...
# Sine burst for the first CLICK_MS of every second, silence the rest.
# Single-quoted like the geq lum= expression above — commas are literal inside
# the quotes, no \, escaping needed (see note near LUM).
CLICK_EXPR="'if(lt(mod(t,1),${CLICK_MS}/1000),${CLICK_AMP}*sin(2*PI*${CLICK_FREQ}*t),0)'"

ffmpeg -y \
  -f lavfi -i "color=c=0x1a1a1a:size=${W}x${H}:rate=${FPS}" \
  -f lavfi -i "aevalsrc=exprs=${CLICK_EXPR}:s=48000" \
  -vf "$VF" \
  -t "$DURATION" \
...
echo ""
echo "How to measure audio latency (e.g. Bluetooth):"
echo "  Each second boundary also fires a ${CLICK_MS}ms ${CLICK_FREQ}Hz click, aligned"
echo "  with the flash. Film screen+speaker together at high fps with a phone, then"
echo "  count the frame gap between the visual flash and the audible click."
