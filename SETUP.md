...
**Open thread, not yet resolved: Bluetooth audio latency.**
- Confirmed the negotiated Bluetooth transport codec is **SBC** (`wpctl inspect 82` / `pw-cli info 82` → `api.bluez5.codec = "sbc"`) — the highest-latency common A2DP codec, and the mandatory baseline every device must support, which is likely why it was picked by default.
...
- **This system's `libspa-0.2-bluetooth` package has no AAC encoder at all** — `dpkg -L libspa-0.2-bluetooth | grep codec` shows plugins only for `aptx, faststream, g722, lc3, ldac, opus-g, opus, sbc` — no `aac`. Almost certainly a Debian licensing exclusion (same reason many ffmpeg builds omit `libfdk-aac`). **This means AAC is not a usable upgrade path here regardless of what the soundbar supports.**
- What IS available and worth pursuing next session: check whether the soundbar advertises **aptX, LDAC, or LC3** support (attempted via `busctl introspect` on the connected device's `MediaEndpoint1` object, but that only exposed PipeWire's own locally-registered SBC endpoint, not the remote device's actual capability list — need a different method, e.g. bluetoothd debug-level logging during a fresh connection, or checking the soundbar's own spec). If one matches, set WirePlumber's bluez5 codec preference to force it instead of SBC.
