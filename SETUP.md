...
### Direct 1-1 Pi connection, no switch (partial fix, 2026-08-21 — needs a manual reboot sometimes)

The two Pis normally connect through a switch with DHCP, but should also work connected directly to each other with just one cable — no switch, no DHCP server, ideally no manual reconfiguration to switch between the two setups. After four physical cable-swap tests, this is only partially true: it works, but not reliably enough to call it "fixed."

**Discovery already worked for this without any code changes**: `config.env`'s `SERVER_IP=pi1.local` uses mDNS (avahi), not a hardcoded IP — mDNS resolves fine over any address scope, including a self-assigned link-local one, since it's designed for exactly this "no router, no DHCP" case. This part was never in question.

**First attempt (didn't work at all): NetworkManager 1.52's `ipv4.link-local=fallback`** on the existing `eth0` profile, documented as "self-assigns a 169.254.x.x address only if no other IPv4 address was obtained." Tested twice live (once alone, once also with `ipv4.may-fail=no` in case that was suppressing it) — in both tests, DHCP timed out as expected but **no link-local address was ever assigned**, and NetworkManager's journal shows zero mention of link-local/ipv4ll activity at all. This appears to be a genuine gap in this exact NetworkManager version (1.52.1 — the very first release of this feature), not a config mistake. Don't rely on this flag as currently used in `setup-autostart.sh`'s `ipv4.link-local fallback` line — it needs replacing with the approach below before the next fresh Pi setup depends on it. **This is a known follow-up, not yet done.**

**Second attempt (partially works): a separate, lower-priority connection profile.** Instead of one profile with a fallback flag, added a second NetworkManager profile bound to the same `eth0` device, dedicated to `ipv4.method=link-local`, with `connection.autoconnect-priority=-999` (the minimum) vs. the primary DHCP profile raised to `0` — an older, more mature NM mechanism (multiple profiles per device, tried in priority order) rather than the newer single-flag approach that failed. Applied live via `nmcli connection add ...` (not yet added to `setup-autostart.sh` — another follow-up).

Physical test results with this approach:
- Never got direct visibility into whether the link-local profile actually activates during the disconnected window — no path to either Pi during that phase, and the volatile journal (this system doesn't persist `journalctl` across reboots — attempted to fix that, didn't succeed, see below) gets wiped whenever a reboot is needed to recover.
- **Recovery back to DHCP once reconnected to the switch is inconsistent.** Across two disconnect/reconnect cycles: once, one Pi recovered automatically with no reboot needed (clean DHCP re-acquisition). Every other time (both Pis on the first cycle, one Pi on the second), it stayed stuck — even a plain unplug/replug at the switch (a fresh carrier-change event, not a full reboot) wasn't enough to make it re-evaluate and prefer the higher-priority DHCP profile again. A full power-cycle reliably fixed it every time it was tried.

**Working conclusion**: treat a full power-cycle as the reliable recovery step any time the network topology changes (switch ↔ direct), rather than expecting automatic recovery. This is a low-cost rule given the whole system already gets power-cycled nightly anyway — but it does mean a topology change *during* a live show would need a manual reboot of whichever Pi doesn't recover on its own, not a hot-swap.

**Also fixed along the way, independent of whether the above ever gets fully sorted**: the master's chrony `allow 192.168.0.0/8` ACL (fixed to the intended `/16` — a `/8` mask only covers the first octet, `192.0.0.0/8`, far broader than intended) didn't cover `169.254.0.0/16` at all. Without that range too, even a working link-local address wouldn't help clock sync — chrony would silently reject the slave's sync requests, timing out after 120s (`wait-ptp-lock.sh`'s `chronyc waitsync` warning: "proceeding anyway") with no obvious error pointing at the real cause. This fix is real and in `setup-autostart.sh` already, independent of the network-layer issues above.

**Unresolved side quest**: tried to make `journalctl` persist across reboots (`Storage=persistent` in `journald.conf`, plus manually creating `/var/log/journal/<machine-id>/` with correct ownership) specifically so a future direct-connect test wouldn't lose its evidence to a recovery reboot. Didn't work — `journalctl --header` kept reporting `/run/log/journal/...` (volatile) even after two service restarts and an explicit `--flush`. Root cause not found; worth another look before the next physical test if deeper NetworkManager debugging is needed again.

**Follow-ups for next time this is picked up**: (1) replace `setup-autostart.sh`'s non-working `ipv4.link-local fallback` line with the dual-profile approach and add the new profile creation to the script; (2) figure out why persistent journald storage isn't sticking; (3) with logs actually working, re-run the physical test to see what's really happening during the disconnected window and why recovery is inconsistent.
