...
### Kodi-on-pi2 diagnostic (2026-09-06/07): RESULT — jitter is the mpv render pipeline, not pi2's hardware
...
**Result: pi2 is smooth under Kodi, at full production bitrate.** `kodi`/`kodi-bin`/`kodi-data` (3:21.3+dfsg-1+rpt2) were already installed on pi2 from an earlier, undocumented session — found running standalone/GBM (confirmed in `kodi.log`: `CApplication::CreateGUI - using the gbm windowing system`), holding a live DRM plane, but as an orphaned manual process (parent PID 1, no systemd unit) with its HTTP JSON-RPC disabled. To drive it programmatically: killed it (`pkill -f "kodi.bin --standalone"`), fixed `~/.kodi/userdata/guisettings.xml` directly (`services.webserver` was `false` despite `advancedsettings.xml` asking for it — **`<webserver>` under `<services>` in `advancedsettings.xml` is not a real key for enabling the webserver in modern Kodi; only `guisettings.xml`'s `services.webserver` controls it**, set `services.webserverauthentication` to `false` too for simplicity), then relaunched detached (`setsid nohup /usr/bin/kodi --standalone &`) so it survives SSH disconnect. No systemd service exists yet — this was a manual/temporary launch for the test.

Drove playback entirely over JSON-RPC (`http://pi2:8080/jsonrpc`): `Player.Open` with a local file path, `Input.ExecuteAction {"action":"codecinfo"}` to show the on-screen decoder/CPU debug overlay, `Input.ExecuteAction {"action":"screenshot"}` + `debug.screenshotpath` (must be set to a writable dir first, e.g. `/home/pipe/screenshots` — the default is empty and screenshots silently fail to save) to pull frames back via `scp` for remote verification. Confirmed via the overlay and `XBMC.GetInfoLabels`: `Player.Process(videodecoder)` = `ff-hevc-drm_prime (HW)`, `pixformat` = `drm_prime` (genuine zero-copy hardware decode, not software fallback), native `3,840x2,160` @ `25.000` FPS.

**Important confound caught and eliminated**: the first test file tried, `video-left.mp4`, is an old ~10Mbps test encode (`ffprobe`: `bit_rate=10037841`) — visibly pixelated on fast motion, not representative. Production content is ~23.3Mbps (confirmed earlier in the Pi4 section). Re-ran against `video-left-2.mp4` (`bit_rate=23296558`, matches) and then directly against the real live production file (`/home/pipe/video.mp4`, 7.4GB, the actual file `config.env` points `client.py` at) — **user confirmed visually, both times: smooth, no jumping, pixelation gone at full bitrate.**

CPU usage the whole time: Kodi's own overlay showed **total CPU ~16%** (`CPU-KODI 16.23-16.85%`) decoding/rendering real 4K/23Mbps production content — starkly lower than the ~270-290% mpv+`zimg` showed on Pi4 for the same class of content (see Problem 1 above), and consistent with the Pi4 investigation's finding that Kodi's DRM PRIME path is genuinely zero-copy where mpv's negotiates down to a CPU-side `zimg` RGB conversion on this SoC generation. `vcgencmd`: `temp=58.7'C`, `throttled=0x0` — clean thermal the entire test, so this isn't a "less CPU because it's about to throttle" artifact either.

**This resolves the open question from the plan below**: pi2's jitter is not a hardware-level (SDRAM/memory-bandwidth) margin problem — a different, cheaper render pipeline made it disappear completely, at the exact content that judders under mpv. That means **the MFG_VER=1/SDRAM-batch hardware theory from the pi3 RMA and pi1/pi2 digression above needs walking back**: pi2 and pi3-original sharing that marker was a real, reproducible correlation, but this result shows the actual mechanism is (most likely) the same mpv/`--vo=drm`/`zimg`-vs-native-YUV-overlay-plane negotiation gap already identified and left unresolved in the Pi4 section ("mpv is routing through zimg into an RGB primary-plane path instead of using the overlay plane's native YUV support directly"). Worth checking: does the *original, RMA'd* pi3 board's replacement, or pi3 itself, show the same "Kodi fixes it" result if retested? That would be the cleanest confirmation this generalizes beyond pi2.

**Confirmed controlled re-test, first-test-monitor confound eliminated**: the initial "smooth under Kodi" result above was captured on a monitor that turned out to only support up to 2560x1440 — lower than pi2's production 3840x2160 output — so it wasn't fully trusted on its own. Re-ran on a second, higher-res monitor (3440x1440 ultrawide) with a real numeric measurement this time, not just visual: `mpv --hwdec=drm-copy --vo=drm --drm-mode=3440x1440@59.97 --video-sync=display-resample --loop`, `mistimed-frame-count` polled over its IPC socket per this project's established method (see "Measurement method" above) — **216 mistimed frames in a 15s window (~375 frames displayed, ~58% mistimed)**, worse than pi2's historical ~27-37%, with clean thermal throughout (`temp=66.4'C`, `throttled=0x0`) and the user confirming "extremely jumpy" on screen. Killed mpv, launched Kodi on the *exact same monitor/mode*, played the same production file — user confirmed: **smooth, even with the image stretched to fit the display's different aspect ratio.** This is the clean, single-variable result the first test couldn't fully claim: same board, same monitor, same DRM connector/mode, same file — only the player differs. **The finding stands confirmed, not just suggestive.**

**mpv mode-string gotcha hit during this re-test, worth keeping**: `--drm-mode=3440x1440@50.00` silently failed (`Could not find mode matching 3440x1440@50.00`) because the monitor's EDID actually reports `49.99Hz`, not exactly `50.00` — mpv's mode matching is exact, not fuzzy. On failure, `--vo=drm` never initializes, video hits EOF instantly, audio keeps playing into a black screen with **no error visible without checking the log** (looks identical to a hung/crashed board from the outside). Always check `mpv --vo=drm --drm-mode=help /dev/null` for the exact advertised Hz before hand-typing a `--drm-mode` value.

**The proposed mpv fix (raw `--hwdec=drm`, true zero-copy, no `-copy` suffix) was tried and fails outright — not a viable quick fix.** Tested directly on pi2: `Looking at hwdec hevc-drm... failed` / `DR failed - disabling.`, immediately before any DRM-mode-matching even happens (so this failure is resolution-independent, not the mode-string gotcha above) — mpv falls back to full **software** decode (`Using zimg` for scaling, 3 CPU threads) rather than negotiating true zero-copy DRM PRIME scanout. This is almost certainly *why* the original build settled on `drm-copy` in the first place — the zero-copy path was already tried and doesn't work with this mpv/ffmpeg/Mesa combination on this SoC generation. Fixing this for real would mean getting `hevc-drm` (non-copy) hwdec to actually initialize — genuine ffmpeg/mpv-level investigation (version bumps, patches), not a flag change.

**Net effect of this session**: the MFG_VER=1/SDRAM-batch hardware theory for pi2 is now solidly falsified, confirmed under controlled conditions, not just a single ambiguous test. The mechanism is mpv's DRM output pipeline specifically, and the "just flip a flag" fix for that pipeline has been tried and ruled out. Remaining paths, unchanged from before: (a) real ffmpeg/mpv patch work on the zero-copy negotiation, (b) build a Kodi-flavored sync client, or (c) retest the original RMA'd pi3 board under Kodi to see how far this generalizes. None started yet — this session ended with pi2's Kodi still running manually (no systemd unit), production `client.py`/mpv autostart still stopped on pi2.
